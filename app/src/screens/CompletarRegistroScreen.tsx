...
export default function CompletarRegistroScreen({ navigation, route }: any) {
	...
	const handleFinalizarRegistro = async () => {
		...
		const materiasSeleccionadas = selectedMateriasIds
			.map(
				(materiaId) =>
					materiasList.find((materia) => String(materia.id) === String(materiaId))
						?.nombre ?? null
			)
							...
							<View style={styles.inputGroup}>
								...
								{errores.materias ? (
									...
									<Text style={styles.inputHint}>
										Puedes seleccionar múltiples materias
									</Text>
								)}
							</View>
						</View>
						...
						<View style={styles.buttonsContainer}>
							...
							>
								{estaCargando ? (
									...
									<RNText style={styles.buttonPrimaryText}>Finalizar Registro</RNText>
								)}
							</PrimaryButton>
