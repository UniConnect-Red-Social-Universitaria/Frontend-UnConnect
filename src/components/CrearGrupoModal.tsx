...
export function CrearGrupoModal({
	...
	const handleCrearGrupo = async () => {
		...
		try {
			const response = await gruposService.crearGrupo({
				nombre: nuevoGrupoNombre.trim(),
				materiaId: nuevaMateriaId,
			});

			if (!response.success) {
				setErrorCreacion(response.message || 'No se pudo crear el grupo');
				return;
			}
			...
			setErrorCreacion(
				err instanceof Error ? err.message : 'Ocurrió un error inesperado al conectar.'
			);
		} finally {
								...
								>
									<Text
										style={[
											...
											nuevaMateriaId === materia.id && styles.materiaChipTextSelected,
										]}
									>
										{materia.nombre}
									</Text>
