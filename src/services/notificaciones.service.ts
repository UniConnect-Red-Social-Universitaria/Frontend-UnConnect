...
let expoGoInfoShown = false;
let notificationsModulePromise: Promise<
    typeof import("expo-notifications") | null
> | null = null;

function logExpoGoInfoOnce() {
    if (expoGoInfoShown) {
        return;
    }

    expoGoInfoShown = true;
    console.log(
        "[Notifications] Expo Go detected: local notifications can work, but remote push is limited. Use a development build for full push support.",
    );
}
...
> {
    if (!notificationsModulePromise) {
        notificationsModulePromise = import("expo-notifications")
            ...
            .catch((error: unknown) => {
                console.error(
                    "[Notifications] Could not load expo-notifications:",
                    error,
                );
                return null;
            });
