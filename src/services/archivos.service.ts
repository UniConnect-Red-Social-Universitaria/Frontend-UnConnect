...
class ArchivosService {
    ...
    async subirArchivo(
        ...
        const baseUrl = apiClient['baseUrl']; // Acceso a propiedad privada para construcción de URL
     ...
     */
    async descargarArchivo(archivoId: string): Promise<string> {
        const token = await apiClient.getToken();
        const baseUrl = apiClient['baseUrl'];

        // Retornar URL con token para descarga directa
        return `${baseUrl}/api/archivos/${archivoId}/descargar?token=${token}`;
    }
