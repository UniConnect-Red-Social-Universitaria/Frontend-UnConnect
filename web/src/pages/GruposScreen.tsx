...
export default function GruposScreen() {
	...
	const handleCrearGrupo = async (e: React.FormEvent) => {
		...
		if (!nuevaMateria.trim()) { setCrearError('La materia es obligatoria.'); return; }
		setCreandoGrupo(true);
		try {
			await gruposService.crearGrupo({ nombre: nuevoNombre.trim(), materiaId: nuevaMateria.trim() });
	...
	return (
		<div style={s.page}>
			...
			{toast && (
				<div style={{ position: 'fixed', bottom: 24, right: 24, zIndex: 9999, backgroundColor: toast.type === 'error' ? '#c0392b' : '#27ae60', color: '#fff', padding: '12px 20px', borderRadius: 10, fontSize: 14, fontWeight: 500, boxShadow: '0 4px 16px rgba(0,0,0,0.18)', animation: 'slideIn 0.25s ease' }}>
					{toast.msg}
				</div>
			)}
			...
			<div style={s.content}>
				...
				{error && !loading && (
					<div style={s.errorBox}>
						⚠️ {error}
						<button className="uc-btn-secondary" onClick={cargarGrupos} style={{ marginLeft: 12, padding: '6px 12px' }}>
							Reintentar
						</button>
					</div>
				)}
					...
					<>
						{misGrupos.length === 0 ? (
							<div style={s.emptyState}>
								...
								<p style={{ fontSize: 16, fontWeight: 600, color: '#00284d', margin: '12px 0 4px' }}>
								...
								{misGrupos.map((g) => (
									...
									>
										<div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
											<h3 style={{ margin: 0, fontSize: 16, fontWeight: 700, color: '#00284d' }}>{g.nombre}</h3>
											<span style={{ fontSize: 10, backgroundColor: '#eafaf1', color: '#1e8449', padding: '3px 8px', borderRadius: 20, fontWeight: 600 }}>
												Miembro ✓
											</span>
										</div>
										...
										<p style={{ margin: 0, fontSize: 12, color: '#7a9ab5' }}>
											{g.cantidadMiembros ?? 0} {g.cantidadMiembros === 1 ? 'miembro' : 'miembros'}
										</p>
									</div>
								))}
							</div>
				...
				<div className="uc-modal-overlay" onClick={() => setShowExplorar(false)}>
					<div className="uc-modal" onClick={(e) => e.stopPropagation()}>
						<div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
							<h2 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: '#00284d' }}>🔍 Grupos disponibles</h2>
							<button onClick={() => setShowExplorar(false)} style={{ background: 'none', border: 'none', fontSize: 22, cursor: 'pointer', color: '#7a9ab5' }}>✕</button>
						</div>
						{gruposDisponibles.length === 0 ? (
							<p style={{ color: '#7a9ab5', textAlign: 'center', padding: '24px 0' }}>No hay grupos disponibles por ahora.</p>
						) : (
							<div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
								{gruposDisponibles.map((g) => {
									const esPendiente = g.estado === 'pendiente';
									return (
										<div key={g.id} style={{ padding: '14px 16px', border: '1px solid #e8eef4', borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
											<div>
												<p style={{ margin: '0 0 4px', fontWeight: 600, color: '#00284d', fontSize: 15 }}>{g.nombre}</p>
												<p style={{ margin: 0, fontSize: 13, color: '#7a9ab5' }}>📚 {g.materia?.nombre} · {g.cantidadMiembros ?? 0} miembros</p>
											</div>
											{esPendiente ? (
												<span style={{ fontSize: 12, color: '#7a9ab5', fontWeight: 500, whiteSpace: 'nowrap' }}>Pendiente...</span>
											) : (
												<button
			...
			{showCrear && (
				<div className="uc-modal-overlay" onClick={() => setShowCrear(false)}>
					<div className="uc-modal" onClick={(e) => e.stopPropagation()}>
						<div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
							<h2 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: '#00284d' }}>➕ Crear grupo</h2>
						...
						<form onSubmit={handleCrearGrupo}>
							<div style={{ marginBottom: 16 }}>
								...
								<input className="uc-form-input" placeholder="Ej. Grupo de Cálculo 1" value={nuevoNombre} onChange={(e) => setNuevoNombre(e.target.value)} disabled={creandoGrupo} />
							</div>
							<div style={{ marginBottom: 16 }}>
								<label style={s.label}>Materia</label>
								{materiasUsuario.length > 0 ? (
									<select className="uc-form-input" value={nuevaMateria} onChange={(e) => setNuevaMateria(e.target.value)} disabled={creandoGrupo}>
										<option value="">Selecciona una materia</option>
										{materiasUsuario.map((m) => <option key={m} value={m}>{m}</option>)}
									</select>
								) : (
									<input className="uc-form-input" placeholder="Ej. Cálculo Diferencial" value={nuevaMateria} onChange={(e) => setNuevaMateria(e.target.value)} disabled={creandoGrupo} />
								)}
							</div>
							{crearError && <p style={{ color: '#e74c3c', fontSize: 13, marginBottom: 12 }}>⚠️ {crearError}</p>}
							<div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
								<button type="button" className="uc-btn-secondary" onClick={() => setShowCrear(false)} disabled={creandoGrupo}>Cancelar</button>
...
const s: Record<string, React.CSSProperties> = {
	...
	errorBox: { backgroundColor: '#fdf0f0', border: '1px solid #f5c6cb', borderRadius: 10, padding: '14px 18px', color: '#c0392b', fontSize: 14, display: 'flex', alignItems: 'center' },
