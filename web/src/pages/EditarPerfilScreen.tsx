...
export default function EditarPerfilScreen() {
  ...
  > = {
    fundador: {
      emoji: "🏆",
      label: "Fundador",
      desc: "Ha creado grupos de estudio",
      color: "#f59e0b",
    },
    "participante-activo": {
      emoji: "⭐",
      label: "Participante Activo",
      desc: "Participa en 3 o más grupos",
      color: "#3b82f6",
    },
    comunicador: {
      emoji: "💬",
      label: "Comunicador",
      desc: "Ha enviado 10 o más mensajes",
      color: "#10b981",
    },
    colaborador: {
      emoji: "🤝",
      label: "Colaborador",
      desc: "Activo en grupos y mensajes",
      color: "#8b5cf6",
    },
  };
  ...
  const cargarPerfilAcademico = useCallback(async () => {
    ...
    try {
      ...
      const catalogoIds = new Set(
        (materiasData || []).map((m) => String(m.id)),
      );
      const idsResueltos = [
        ...new Set(
  ...
  const handleGuardar = async () => {
    ...
    try {
      ...
      const canalesSeleccionados: CanalNotificacion[] = (
        Object.entries(preferenciasNotificaciones) as [
          CanalNotificacion,
          boolean,
        ][]
      )
      ...
      <div style={s.content}>
        ...
        {cargando ? (
          <div style={s.centered}>
            <div style={s.spinner} />
            <p style={{ color: "#7a9ab5", marginTop: 12 }}>
              Cargando perfil...
            </p>
          </div>
        ) : (
          <div style={s.card}>
            ...
            <div style={s.inputGroup}>
              ...
              />
              {errores.semestre && (
                <p style={s.errorText}>{errores.semestre}</p>
              )}
            </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div style={s.badgeInfo}>
                            <p style={{ ...s.badgeLabel, color: info.color }}>
                              {info.label}
                            </p>
                            <p style={{ ...s.badgeDesc, color: info.color }}>
                              {info.desc}
                            </p>
                          </div>
                        </div>
            ...
            <div style={s.section}>
              <p style={s.sectionTitle}>📬 Canales de Notificaciones</p>
              {CANALES.map((canal) => (
                <label key={canal.id} style={s.notificacionLabel}>
                  <input
                    type="checkbox"
                    checked={preferenciasNotificaciones[canal.id]}
                    onChange={(e) => {
...
const s: Record<string, React.CSSProperties> = {
  ...
  spinner: {
    ...
    borderTopColor: theme?.colors?.primary || "#003e70",
    borderRadius: "50%",
  ...
  verMasBtn: {
    ...
    padding: "10px 14px",
    marginTop: 14,
    backgroundColor: theme?.colors?.primary || "#003e70",
    color: "#fff",
    border: `1.5px solid ${theme?.colors?.primary || "#003e70"}`,
    borderRadius: 20,
    fontSize: 13,
    fontWeight: 600,
    cursor: "pointer",
  ...
  statNum: {
    fontSize: 24,
    fontWeight: 700,
    color: theme?.colors?.primary || "#003e70",
    margin: 0,
  },
  statLabel: {
    fontSize: 11,
  ...
  notificacionCheckbox: {
    width: 18,
    height: 18,
    cursor: "pointer",
    accentColor: theme?.colors?.primary || "#003e70",
  },
};
